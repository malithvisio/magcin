...
const useTicketFilter = (ticketsData: Ticket[]) => {
  ...
  const uniqueAirlines = [
    ...new Set(ticketsData.map(ticket => ticket.airlines)),
  ];
  ...
  const filteredTickets = ticketsData.filter(ticket => {
    return (
      ...
      (filter.locations.length === 0 ||
        filter.locations.includes(ticket.location)) &&
      ticket.price >= filter.priceRange[0] &&
      ticket.price <= filter.priceRange[1] &&
      (filter.ratings.length === 0 || filter.ratings.includes(ticket.rating)) &&
      (filter.airlines.length === 0 ||
        filter.airlines.includes(ticket.airlines))
    );
  });
