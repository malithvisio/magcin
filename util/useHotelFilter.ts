...
const useHotelFilter = (hotelsData: Hotel[]) => {
  ...
  const uniqueRoomStyles = [
    ...new Set(hotelsData.map(hotel => hotel.roomStyle)),
  ];
  const uniqueAmenities = [
    ...new Set(hotelsData.map(hotel => hotel.amenities)),
  ];
  ...
  const uniqueHotelsType = [
    ...new Set(hotelsData.map(hotel => hotel.hotelType)),
  ];

  const filteredHotels = hotelsData.filter(hotel => {
    return (
      (filter.names.length === 0 || filter.names.includes(hotel.name)) &&
      (filter.roomStyle.length === 0 ||
        filter.roomStyle.includes(hotel.roomStyle)) &&
      (filter.amenities.length === 0 ||
        filter.amenities.includes(hotel.amenities)) &&
      (filter.locations.length === 0 ||
        filter.locations.includes(hotel.location)) &&
      ...
      (filter.hotelType.length === 0 ||
        filter.hotelType.includes(hotel.hotelType))
    );
  });
